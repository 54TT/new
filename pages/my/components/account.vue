<template>
	<view class="accountBoxMore">
		<div class="topBox">
			<TopBack text="我的账户" url="/pages/my/index" isTabbar="true" />
		</div>
...
const goUrl = (key) => {
	...
	if (key === 'trade') {
		uni.navigateTo({
			url: '/pages/my/components/password/tradePassword'
		});
	} else if (key === 'account') {
		uni.navigateTo({
			url: '/pages/my/components/password/accountPassword'
		});
	} else if (key === 'pay') {
		console.log(1111111111111);
		uni.navigateTo({
			url: '/pages/my/components/pay'
		});
	}
};
